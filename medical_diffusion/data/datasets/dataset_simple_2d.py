...
class CheXpert_2_Dataset(SimpleDataset2D):
    def __init__(self, *args, image_resize=512,**kwargs):
        super().__init__(*args, **kwargs)
        self.transform = transforms.Compose([
            transforms.Resize((image_resize, image_resize)),  # 调整图像大小
            transforms.ToTensor(),
        ])
        # 读取标签文件，假设 ';' 为分隔符
        labels = pd.read_csv(self.path_root / 'labels/cheXPert_label.csv', index_col=['Path', 'Image Index'],
                             delimiter=';')

        # 保留 fold 为 'train' 的行，并删除 fold 列
        labels = labels.loc[labels['fold'] == 'train'].copy()
        labels = labels.drop(columns=['fold'])
    ...
    def __getitem__(self, index):
        ...
        # 直接使用标签文件中的 Path 列作为路径
        path_item = Path(path_index)  # 注意，不再拼接 self.path_root
        img = self.load_item(path_item)
        uid = image_index
